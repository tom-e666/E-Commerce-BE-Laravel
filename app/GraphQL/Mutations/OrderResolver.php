...
final readonly class OrderResolver
{
    ...
    {
        // Eager load the products for all order items to avoid N+1 issues
        $order->load('items.product');
        
        // Get product IDs to fetch MongoDB details in batch
        $productIds = $order->items->pluck('product_id')->toArray();
        
        // Fetch all product details in one MongoDB query
